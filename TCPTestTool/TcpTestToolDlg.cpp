...
BOOL CTcpTestToolDlg::OnInitDialog()
{
	...
	m_listOutput.InsertColumn(col_index, _T("序号"), LVCFMT_LEFT, 50);
	m_listOutput.InsertColumn(col_threadid, _T("线程id"), LVCFMT_LEFT, 80);
	m_listOutput.InsertColumn(col_maxtime, _T("最大响应时间ms"), LVCFMT_LEFT, 150);
	m_listOutput.InsertColumn(col_mintime, _T("最小响应时间ms"), LVCFMT_LEFT, 150);
	m_listOutput.InsertColumn(col_times, _T("测试次数"), LVCFMT_LEFT, 100);
	m_listOutput.InsertColumn(col_sendonce, _T("单次发送"), LVCFMT_LEFT, 100);
	m_listOutput.InsertColumn(col_recvonce, _T("单次接收"), LVCFMT_LEFT, 100);
	m_listOutput.InsertColumn(col_sendall, _T("发送总数"), LVCFMT_LEFT, 100);
...
// 获取当前时间（默认格式为2019-07-27 12:32:20）
...
void CTcpTestToolDlg::Log(const char* format, ...)
{
	...
	// 获取日志生成的当前时间（格式为2019-07-27 12:32:20）
	/*char currentTime[64] = { 0 };
	SYSTEMTIME st;
	::GetLocalTime(&st);
